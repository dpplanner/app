...
class _ClubTimetablePageState extends State<ClubTimetablePage> {
  ...
  Future<List<ResourceModel>> getReservations() async {
    try {
      // 클럽 자원 불러오기
      List<List<ResourceModel>> resources = await ResourceApiService.getResources();
      ...
      endOfResource = DateTime(endOfResource.year, endOfResource.month, endOfResource.day)
      ...
      DateTime endDate = !(MemberController.to.clubMember().role == "ADMIN" ||
              ...
              ((endOfResource.isBefore(endOfWeek) || endOfResource.isAfter(startOfWeek)))
          ? endOfResource
          : endOfWeek;

      if (startOfWeek.isBefore(endDate)) {
        List<ReservationModel> reservations = await ReservationApiService.getReservations(
        ...
        for (var i in reservations) {
          events.add(CalendarEventData(
              ...
              endTime: DateTime.parse(i.endDateTime).subtract(const Duration(microseconds: 1)),
        ...
        for (var i in locks) {
          ...
          for (DateTime j = startDate; j.isBefore(endDate); j = j.add(const Duration(days: 1))) {
            DateTime startTime =
                (j.year == startDate.year && j.month == startDate.month && j.day == startDate.day)
                    ? startDate
                    : DateTime(startDate.year, startDate.month, startDate.day);
            DateTime endTime =
                (j.year == endDate.year && j.month == endDate.month && j.day == endDate.day)
                    ? endDate
      ...
      if (!(MemberController.to.clubMember().role == "ADMIN" ||
        ...
        for (DateTime j = endOfResource;
          ...
          events.add(CalendarEventData(
              date: j,
              startTime: DateTime.parse(DateFormat('yyyy-MM-dd 00:00:00').format(j)),
              endTime: DateTime.parse(DateFormat('yyyy-MM-dd 23:59:59').format(j)),
              title: "",
              description: "",
              color: AppColor.lockColor.withOpacity(0.5)));
        }
    ...
    return Scaffold(
        ...
        body: SafeArea(
          child: FutureBuilder(
              ...
              builder: (BuildContext context, AsyncSnapshot<List<ResourceModel>> snapshot) {
                        ...
                        child: Stack(
                          children: [
                            Row(
                              children: [
                                Flexible(
                                    flex: 11, child: Container(color: AppColor.backgroundColor)),
                                Flexible(
                                    flex: 85, child: Container(color: AppColor.backgroundColor2)),
                                Flexible(
                            ...
                            Center(
                              child: WeekView(
                                      ...
                                      Expanded(
                                        child: Padding(
                                          padding: EdgeInsets.only(
                                              ...
                                              right: SizeController.to.screenWidth * 0.1),
                                          child: WeekPageHeader(
                                            ...
                                            headerStyle: HeaderStyle(
                                                ...
                                                leftIcon: GestureDetector(
                                                    ...
                                                    child: const Icon(SFSymbols.calendar,
                                ...
                                eventTileBuilder: (date, events, boundry, start, end) {
                                  if (events.isNotEmpty) {
                                    return Container(
                                      decoration: BoxDecoration(
                                        border: Border.all(
                                          color: events[0].color == AppColor.subColor4 &&
                                              ...
                                              : Colors.transparent,
                                          width: events[0].color == AppColor.subColor4 &&
                                                  (MemberController.to.clubMember().role ==
                                                          "ADMIN" ||
                                                      (MemberController.to
                                                                  .clubMember()
                                    ...
                                    try {
                                      ReservationModel reservation =
                                          await ReservationApiService.getReservation(
                                              reservationId:
                                                  int.parse(events[0].title.split(" ")[0]));
                                ...
                                onPageChange: (DateTime firstDayOfWeek, int daysInWeek) {
                                  setState(() {
  ...
  Future<void> addReservation(
      {required int types, required ReservationModel? reservation, DateTime? chooseDate}) async {
    ...
    Color selectedColor =
        reservation == null ? AppColor.reservationColors[0] : AppColor.ofHex(reservation.color);
    ...
    if (types == 3) {
      ...
      try {
        ClubMemberModel reservationOwner = await ClubMemberApiService.getClubMember(
            clubId: ClubController.to.club().id, clubMemberId: reservation.clubMemberId);
        owner = {'clubMemberName': reservationOwner.name, 'clubMemberId': reservationOwner.id};
      } catch (e) {
    ...
    Future<List<LockModel>> getLockList(StateSetter setState) async {
      try {
        DateTime startOfMonth = DateTime(startOfWeek.year, startOfWeek.month, 1);
        DateTime endOfMonth = DateTime(startOfWeek.year, startOfWeek.month + 1, 0);
        String startTimeLock = DateFormat('yyyy-MM-dd 00:00:00').format(startOfMonth);
        String endTimeLock =
            DateFormat('yyyy-MM-dd 00:00:00').format(endOfMonth.add(const Duration(days: 1)));
        List<LockModel> locks = await LockApiService.getLocks(
            resourceId: selectedValue!.id, startDateTime: startTimeLock, endDateTime: endTimeLock);
        return locks;
      } catch (e) {
        print(e.toString());
      }
    ...
    Future<List<ClubMemberModel>> getMemberList(StateSetter setState) async {
      try {
        List<ClubMemberModel> members = await ClubMemberApiService.getClubMemberList(
            clubId: ClubController.to.club().id, confirmed: true);
        List<ClubMemberModel> removeMeMembers = members;
        if (types == 9) {
          removeMeMembers =
              members.where((member) => member.id != MemberController.to.clubMember().id).toList();
        }
                                        ...
                                        onTap: () {
                                          if (types == 3) {
                                            ...
                                            snackBar(
                                                ...
                                                content: "날짜를 변경하고 싶으시다면 새로 예약해주세요");
                                          } else {
                                            setState(() {
                                              selectedDay = reservationTime;
                                              lastPages.add(0);
                                      ...
                                      onTap: () async {
                                        if (types == 3) {
                                          ...
                                          snackBar(
                                              ...
                                              content: "시간을 변경하고 싶으시다면 새로 예약해주세요");
                                        } else {
                                          unableTime.clear();
                                          ...
                                          if (!(MemberController.to.clubMember().role == "ADMIN" ||
                                            ...
                                            if (reservationTime.isBefore(DateTime.now()) ||
                                                reservationTime.day == DateTime.now().day) {
                                              for (var j = 0; j <= DateTime.now().hour; j++) {
                                                unableTime.add(j);
                                              }
                                            }
                                          }
                                          ...
                                          List<ReservationModel> reservations =
                                              await ReservationApiService.getReservations(
                                                  resourceId: selectedValue!.id,
                                                  startDateTime: DateFormat('yyyy-MM-dd 00:00:00')
                                                      .format(reservationTime),
                                          ...
                                          for (var i in reservations) {
                                            if (i.reservationId != reservation?.reservationId) {
                                              int start =
                                                  int.parse(i.startDateTime.substring(11, 13));
                                              int end = int.parse(i.endDateTime.substring(11, 13));
                                              if (end == 0) {
                                          ...
                                          List<LockModel> locks = await LockApiService.getLocks(
                                              ...
                                              startDateTime: DateFormat('yyyy-MM-dd 00:00:00')
                                                  .format(reservationTime),
                                              endDateTime: DateFormat('yyyy-MM-dd 00:00:00').format(
                                                  reservationTime.add(const Duration(days: 1))));

                                          for (var i in locks) {
                                            DateTime startDate = DateTime.parse(i.startDateTime);
                                            DateTime endDate = DateTime.parse(i.endDateTime);
                                            bool isStart = startDate.year == reservationTime.year &&
                                                startDate.month == reservationTime.month &&
                                                startDate.day == reservationTime.day;
                                            bool isEnd = endDate.year == reservationTime.year &&
                                                endDate.month == reservationTime.month &&
                                                endDate.day == reservationTime.day;
                                            int start = 0;
                                            int end = 24;
                                            if (isStart && isEnd) {
                                              start = int.parse(i.startDateTime.substring(11, 13));
                                              end = int.parse(i.endDateTime.substring(11, 13));
                                            } else if (isStart) {
                                              start = int.parse(i.startDateTime.substring(11, 13));
                                              end = 24;
                                            } else if (isEnd) {
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          GestureDetector(
                                            ...
                                            child: Text(
                                              (types == 0 || types == 4) && invitees.isEmpty
                                                  ? "선택하기"
                                                  : invitees.isNotEmpty && invitees.length >= 2
                                                      ? "${invitees[0]["clubMemberName"]} 외 ${invitees.length - 1}명"
                                                      : invitees
                                                          .map((invitee) =>
                                                              invitee["clubMemberName"] as String)
                                                          .join(", "),
                                              style: TextStyle(
                                    ...
                                    if (reservation != null &&
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          ...
                                          if (checkedReturn &&
                                              reservation.attachmentsUrl.isNotEmpty)
                                            Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                const Padding(
                            ...
                            replacement: Column(
                              children: [
                                ...
                                calendar.TableCalendar(
                                  ...
                                  daysOfWeekHeight: SizeController.to.screenHeight * 0.03,
                                  ...
                                  headerStyle: const calendar.HeaderStyle(
                                      headerPadding: EdgeInsets.fromLTRB(24, 24, 24, 12),
                                      leftChevronIcon:
                                          Icon(SFSymbols.chevron_left, color: AppColor.textColor),
                                      rightChevronIcon:
                                          Icon(SFSymbols.chevron_right, color: AppColor.textColor),
                                    ...
                                    setState(() {
                                      ...
                                      if (types == 5) {
                                        ...
                                        if (MemberController.to.clubMember().role == "ADMIN" ||
                                          ...
                                          if (newSelectedDay.isAfter(rangeStart) &&
                            ...
                            child: Visibility(
                              ...
                              replacement:
                                ...
                                Text(
                                  types == 2 && lastPages.last == 0
                                      ? "예약할 시간을 선택해주세요"
                                      : types == 2 && lastPages[lastPages.length - 2] != 2
                                          ? "잠금 시작 시간을 선택해주세요"
                                          : "잠금 종료 시간을 선택해주세요",
                                  style: const TextStyle(
                                      color: AppColor.textColor,
                                          ...
                                          if (unableTime.contains(index)) {
                                            ...
                                            setState(() {
                                              if (checkedTime.isNotEmpty) {
                                                timeButton[checkedTime.first] = false;
                                                checkedTime.clear();
                                              }
                                              timeButton[index] = true;
                                              checkedTime.add(index);
                                        ...
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: unableTime.contains(index)
                                                ? AppColor.markColor.withOpacity(0.7)
                                                : timeButton[index]
                                                    ? AppColor.objectColor
                                                    : AppColor.backgroundColor2,
                                          ...
                                          child: Center(
                                            child: Text(
                                              types == 2 && lastPages.last == 0
                                                  ? '$index:00'
                                                  : types == 2 &&
                                                          lastPages[lastPages.length - 2] != 2
                                                      ? '$index:00'
                                                      : '${index + 1}:00',
                                              style: TextStyle(
                                ...
                                if (checkedTime.isNotEmpty)
                                  Padding(
                                    ...
                                    child: Center(
                                      child: Text(
                                        types == 2 && lastPages.last == 0 && checkedTime.length == 1
                                      ...
                                      children: [
                                        ...
                                        Expanded(
                                          child: SizedBox(
                                            ...
                                            child: ListView(
                                              scrollDirection: Axis.horizontal,
                                              children: selectedImages.map<Widget>((image) {
                                                return Stack(
                                                  children: [
                                                    Padding(
                                                      padding: const EdgeInsets.only(right: 8.0),
                                                        ...
                                                        Row(
                                                          ...
                                                          children: [
                                                            ...
                                                            Column(
                                                              children: [
                                                                Text(
                                                                  DateFormat("yy년 MM월 dd일 H:00 부터",
                                                                          'ko_KR')
                                                                ...
                                                                Text(
                                                                  DateFormat("yy년 MM월 dd일 H:00 까지",
                                                                          'ko_KR')
                                                                      .format(DateTime.parse(
                                                                          snapshot.data![index]
                                                                              .endDateTime)),
                                                                  ...
                                                                  onPressed: () {
                                                                    setState(() {
                                                                      ...
                                                                      lockStartDate =
                                                                          DateTime.parse(
                                                                              lock!.startDateTime);
                                                                      lockEndDate = DateTime.parse(
                                                                          lock!.endDateTime);
                                                                      lockStartTime = int.parse(
                                                                          lock!.startDateTime
                                                                              .substring(11, 13));
                                          ...
                                          children: [
                                            ...
                                            GestureDetector(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    DateFormat("yy년 MM월 dd일 $lockStartTime:00 부터",
                                                            'ko_KR')
                                                  ...
                                                  Text(
                                                    DateFormat("yy년 MM월 dd일 $lockEndTime:00 까지",
                                                            'ko_KR')
                                                        .format(lockEndDate),
                                                    style: const TextStyle(
                                                        fontSize: 15,
                                                ...
                                                child: Column(
                                                  children:
                                                    ...
                                                    if (types == 9) {
                                                      ...
                                                      if (updateOwner["clubMemberId"] ==
                                                              snapshot.data![index].id &&
                                                          updateOwner["clubMemberName"] ==
                                                              snapshot.data![index].name) {
                                                        isChecked.add(true);
                                                      } else {
                                                        isChecked.add(false);
                  ...
                  Padding(
                    ...
                    child: Visibility(
                      ...
                      replacement: NextPageButton(
                        ...
                        buttonColor: isChecked1 ? AppColor.objectColor : AppColor.subColor3,
                        onPressed: () async {
                          if (checkedTime.isNotEmpty) {
                            try {
                              String startDateTime = (0 <= startTime && startTime <= 9)
                                  ? DateFormat("yyyy-MM-dd 0$startTime:00:00", 'ko_KR')
                                      .format(reservationTime)
                                  : DateFormat("yyyy-MM-dd $startTime:00:00", 'ko_KR')
                                      .format(reservationTime);
                              String endDateTime = (0 <= endTime && endTime <= 9)
                                  ? DateFormat("yyyy-MM-dd 0$endTime:00:00", 'ko_KR')
                                      .format(reservationTime)
                                  : DateFormat("yyyy-MM-dd $endTime:00:00", 'ko_KR')
                                      .format(reservationTime);
                              List<int> clubMemberIds = invitees
                                  .map((invitee) => invitee["clubMemberId"] as int)
                                  .toList();
                              if (types == 0) {
                                ...
                                await ReservationApiService.putReservation(
                                    ...
                                    endDateTime: endDateTime,
                                    reservationInvitees: clubMemberIds);
                                await ReservationApiService.patchReservationOwner(
                                    reservationId: reservation.reservationId,
                                    reservationOwnerId: owner['clubMemberId']);
                              }
                              getReservations();
                              ...
                              print(e.toString());
                              if (types == 0) {
                                snackBar(title: "예약 신청에 문제가 발생하였습니다", content: "관리자에게 문의해주세요");
                              } else {
                                snackBar(title: "예약 수정에 문제가 발생하였습니다", content: "관리자에게 문의해주세요");
                              }
                            }
                          }
                        },
                      ...
                      child: Visibility(
                          ...
                          replacement: Column(
                            children: [
                              if ((reservation?.clubMemberId ==
                                      MemberController.to.clubMember().id) &&
                                  reservation != null &&
                                  DateTime.parse(reservation.endDateTime).isBefore(now) &&
                                  !reservation.returned)
                                NextPageButton(
                                  text: const Text(
                                    "반납하기",
                                      ...
                                      Expanded(
                                        child: Visibility(
                                          visible: reservation?.clubMemberId ==
                                                  MemberController.to.clubMember().id &&
                                              reservation != null &&
                                              DateTime.parse(reservation.endDateTime).isAfter(now),
                                          replacement: NextPageButton(
                                            text: const Text(
                                              "예약 삭제하기",
                                              style: TextStyle(
                                          ...
                                          child: NextPageButton(
                                            ...
                                            onPressed: () async {
                                              await checkDeleteReservation(
                                                  id: reservation!.reservationId, types: 1);
                                            },
                                          ),
                                        ),
                                      ),
                              ...
                              children: [
                                Expanded(
                                  child: NextPageButton(
                                    ...
                                    buttonColor: AppColor.subColor3,
                                    onPressed: () {
                                      rejectReservation(id: reservation!.reservationId);
                                    },
                                  ),
                                ),
                                const SizedBox(
                                  ...
                                  child: NextPageButton(
                                    ...
                                    onPressed: () async {
                                      try {
                                        ...
                                        print(e.toString());
                                        snackBar(
                                            title: "예약을 승인하지 못했습니다", content: "잠시 후 다시 시도해 주세요");
                                      }
                                    },
                                  ),
                                ),
                        ...
                        child: Visibility(
                          ...
                          replacement: NextPageButton(
                            ...
                            onPressed: () async {
                              try {
                                ReservationModel temp = await ReservationApiService.postReturn(
                                    reservationId: reservation!.reservationId,
                          ...
                          child: Visibility(
                            visible: !(types == 7 || (types == 8 && lock == null)),
                            replacement: NextPageButton(
                              text: const Text(
                                "잠금 시간 추가하기",
                                style: TextStyle(
                                ...
                                if (types == 7) {
                                  ...
                                  try {
                                    String startDateTime = (0 <= lockStartTime &&
                                            lockStartTime <= 9)
                                        ? DateFormat("yyyy-MM-dd 0$lockStartTime:00:00", 'ko_KR')
                                            .format(lockStartDate)
                                        : DateFormat("yyyy-MM-dd $lockStartTime:00:00", 'ko_KR')
                                            .format(lockStartDate);
                                    String endDateTime = (0 <= lockEndTime && lockEndTime <= 9)
                                        ? DateFormat("yyyy-MM-dd 0$lockEndTime:00:00", 'ko_KR')
                                            .format(lockEndDate)
                                        : DateFormat("yyyy-MM-dd $lockEndTime:00:00", 'ko_KR')
                                            .format(lockEndDate);
                                    LockModel temp = await LockApiService.postLock(
                                        resourceId: selectedValue!.id,
                                        startDateTime: startDateTime,
                                        endDateTime: endDateTime,
                                    ...
                                    print(e.toString());
                                    snackBar(title: "잠금 시간 추가가 불가능합니다", content: "시간을 조정해주세요");
                                  }
                                }
                              },
                            ),
                            child: Visibility(
                              ...
                              replacement: NextPageButton(
                                ...
                                onPressed: () async {
                                  try {
                                    ...
                                    String endDateTime = '';
                                    if (types == 8 && lock != null && lastPages.last != -1) {
                                      startDateTime = lock!.startDateTime;
                                      endDateTime = lock!.endDateTime;
                                    } else {
                                      startDateTime = (0 <= lockStartTime && lockStartTime <= 9)
                                          ? DateFormat("yyyy-MM-dd 0$lockStartTime:00:00", 'ko_KR')
                                              .format(lockStartDate)
                                          : DateFormat("yyyy-MM-dd $lockStartTime:00:00", 'ko_KR')
                                              .format(lockStartDate);
                                      endDateTime = (0 <= lockEndTime && lockEndTime <= 9)
                                          ? DateFormat("yyyy-MM-dd 0$lockEndTime:00:00", 'ko_KR')
                                              .format(lockEndDate)
                                          : DateFormat("yyyy-MM-dd $lockEndTime:00:00", 'ko_KR')
                                              .format(lockEndDate);
                              ...
                              child: NextPageButton(
                                ...
                                buttonColor: types == 1 || types == 9 || types == 10
                                    ? AppColor.objectColor
                                    : isChecked2
                                        ? AppColor.objectColor
                                        : AppColor.subColor3,
                                  ...
                                  if (types == 1) {
                                    if (lastPages.last == 0) {
                                      setState(() {
                                        ...
                                        lastPages.last == 2) {
                                      setState(() {
                                        if (lastPages.last == 7 || lastPages.last == 8) {
                                    ...
                                    if (lastPages.last == 0) {
                                    ...
                                    } else if (lastPages[lastPages.length - 2] != 2) {
                                      setState(() {
                                        updateStartTime = checkedTime[0];
                                        selectedDay = lockEndDate != updateStartDate
                                            ? updateStartDate
                                            : lockEndDate;
                                        focusedDay = lockEndDate != updateStartDate
                                            ? updateStartDate
                                            : lockEndDate;
                                        lastPages.add(2);
                                        types = 1;
                                      ...
                                      setState(() {
                                        ...
                                        updateEndDate = now;
                                        updateStartTime = -1;
                                        if (lastPages[1] == 8 && lastPages.first != -1) {
                                          lastPages.clear();
                                          lastPages.add(7);
                                          lastPages.add(-1);
                                        } else {
                                    ...
                                    getReservations();
                                    weekViewStateKey.currentState?.jumpToWeek(selectedDate);
                                    Get.back();
                                  } else if (types == 9) {
                                    setState(() {
                                      invitees.clear();
                                    ...
                                    snackBar(title: "시간을 선택하지 않았습니다", content: "최소 한시간 이상 선택해주세요");
                                  }
                                },
                              ),
                            ),
          ...
          Padding(
            ...
            child: NextPageButton(
              text: const Text(
                "위 공지에 동의하기",
                style: TextStyle(
                    fontSize: 16, fontWeight: FontWeight.w700, color: AppColor.backgroundColor),
              ),
              buttonColor: AppColor.objectColor,
              onPressed: () {
                Get.back();
  ...
  /// types == 1 : 취소
  /// types == 2 : 잠금 삭제
  Future<void> checkDeleteReservation({required int types, required int id}) async {
            ...
            children: [
              Padding(
                ...
                child: NextPageButton(
                  ...
                  onPressed: () async {
                    try {
                      if (types == 0) {
                        await ReservationApiService.deleteReservation(reservationId: id);
                        getReservations();
                        Get.back(result: true);
                        Get.back();
                      } else if (types == 1) {
                        await ReservationApiService.cancelReservation(reservationId: id);
                        getReservations();
                        Get.back(result: true);
                        Get.back();
                      } else {
              ...
              TextButton(
                ...
                child: const Text(
                  "닫기",
                  style: TextStyle(
                      fontSize: 12, fontWeight: FontWeight.w500, color: AppColor.textColor2),
                ),
              ),
            ],
          ),
